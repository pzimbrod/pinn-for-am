...
test_num_boundary       =   [8000, 16000]
test_num_initial        =   [4000, 8000]
test_nn_layers          =   3
...
files = glob.glob("*_trainingspunkte_bash.csv")
i = len(files)
...
ergebnis = open(str(i) + '_trainingspunkte_bash.csv', 'w')
writer = csv.writer(ergebnis)
writer.writerow(header)

start_time = tm.time()
data = dde.data.TimePDE(geomtime, pde, [bc_a_x0, bc_a_x1, bc_a_y0, bc_a_y1, bc_b_x0, bc_b_x1, bc_b_y0, bc_b_y1, ic_a, ic_b], num_domain=test_num_domain[j], num_boundary=test_num_boundary[j], num_initial=test_num_initial[j], train_distribution=test_train_distribution)
...
auswertung = (test_train_distribution, test_num_domain[j], test_num_boundary[j], test_num_initial[j], test_activation, test_nn_layers, time_taken, model.losshistory.steps[-1], steps_adam, steps_LBFSG,  loss_0, loss_1)
writer.writerow(auswertung)

vis.make_image(0, 'IC_TEST_2', model, 0)
