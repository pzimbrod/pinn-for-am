...
ergebnis = open('ergebnisse_epochen.csv', 'w')
...
while i < itterationen:
    ...
    dde.model.optimizers.config.set_LBFGS_options(
        maxcor=100,
        ftol= 0, #1e-12,
        gtol= ((1.0 * np.finfo(float).eps)**0.5),
        maxiter=ep_lb,
        maxfun=ep_lb,
        maxls=50,)
    ...
    #auswertung = ('loss 0',loss_0, 'loss 1', loss_1, 'zeit', time_taken)
    auswertung = loss_0, loss_1

   

    with open("loss.dat") as f:
        with open(str(i) + "_loss.csv", "w") as f1:
            for line in f:
                f1.write(line)
            f1.write(str(loss_0) + ' ' + str(time_taken))

    if i == 1:
        loss_vis = loss_0_matrix
    else:
        loss_vis = loss_vis + loss_0_matrix

    if i == itterationen:
        loss_vis = loss_vis/itterationen 
        vis.plot_matrix(loss_vis, 'Loss_Epochen_Durchschnitt')
