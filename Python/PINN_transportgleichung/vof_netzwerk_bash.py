...
files = glob.glob("*_netzwerk_bash.csv")
i = len(files)
...
ergebnis = open(str(i) + '_netzwerk_bash.csv', 'w')
...
net = dde.nn.FNN([3] + [32] * test_nn_layers[j] + [2], test_activation, "Glorot normal")
...
auswertung = (test_train_distribution, test_num_domain, test_num_boundary, test_num_initial, test_activation, test_nn_layers[j], time_taken, model.losshistory.steps[-1], steps_adam, steps_LBFSG,  loss_0, loss_1)
