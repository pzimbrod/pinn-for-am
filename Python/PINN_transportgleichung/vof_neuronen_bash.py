...
files = glob.glob("*_neuronen_bash.csv")
i = len(files)

j = i % 2

#CSV
header = ('train distribution', 'domain', 'boundary', 'initial', 'activation', 'neuronen', 'time taken', 'steps', 'steps_adam', 'steps_L-BFSG', 'loss 0', 'loss 1')
ergebnis = open(str(i) + '_neuronen_bash.csv', 'w')
...
net = dde.nn.FNN([3] + [test_nn_neuronen[j]] * 3 + [2], test_activation, "Glorot normal")
...
loss_0, loss_0_matrix, pred_0 = vis.get_mse(vis.get_solution, model, 5, 0)
loss_1, loss_1_matrix, pred_1 = vis.get_mse(vis.get_solution, model, 5, 1)
auswertung = (test_train_distribution, test_num_domain, test_num_boundary, test_num_initial, test_activation, test_nn_neuronen[j], time_taken, model.losshistory.steps[-1], steps_adam, steps_LBFSG,  loss_0, pred_0)
