...
act_Fkt = tanh #Flux.relu, Flux.sigmoid, Flux.Gelu, tanh
...
function create_PINN(eq, bcs, domains, chain, inner, hidden)  
    ...
    #CSV-Datei erstellen
    name = "/home/pi/stolzean/ErgebnissePINNs/PINN_Tanh_nodes$(inner)_hidden$(hidden).csv"
    writedlm(name, Loss, ',') 
...
for inner in inner_array
    println("Tanh: Prozess startet mit $(inner)!")
