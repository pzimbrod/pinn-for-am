...
act_Fkt = Flux.gelu #Flux.relu, Flux.sigmoid, Flux.Gelu, tanh
...
function create_PINN(eq, bcs, domains, chain, inner, hidden)  
    ...
    #CSV-Datei erstellen
    name = "/home/pi/stolzean/ErgebnissePINNs/PINN_Gelu_nodes$(inner)_hidden$(hidden).csv"
    writedlm(name, Loss, ',') 
...
for inner in inner_array
    println("GeLu: Prozess startet mit $(inner)!")
